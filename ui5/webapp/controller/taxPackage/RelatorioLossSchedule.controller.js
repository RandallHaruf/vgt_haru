sap.ui.define([
	...
	return BaseController.extend("ui5ns.ui5.controller.taxPackage.RelatorioLossSchedule", {
		...
		onExit: function () {
			...
			that.getModel().setProperty("/NomeReport",that.getResourceBundle().getText("viewGeralRelatorio") + " " + that.getResourceBundle().getText("viewTaxpackageEdiçãoTrimestreLOSSSCHEDULE"));
		...
		getSelectedItemsTemplate: function(oEvent){
			...
			oWhere.AnoFiscal = oFYSelecionadas;	
		...
		onTemplateGet: function (oEvent) {
			...
			this.getModel().setProperty("/FYSelecionadas", forcaSelecao.AnoFiscal);	
		...
		_atualizarDados: function () {
			...
			if(oWhere.Empresa === null){
				...
				jQuery.ajax(Constants.urlBackend + "DeepQueryDistinctLOSSSCHEDULE/ReportTaxPackage?full=" + (this.isIFrame() ? "true" : "false")+"&moduloAtual=2" /*Modulo 2 representa Tax Package*/, {
					type: "POST",
					xhrFields: {
			...
			if(oWhere.AnoFiscal === null){
				oWhere.Distinct = ["tblSchedule.fy"];
				jQuery.ajax(Constants.urlBackend + "DeepQueryDistinctLOSSSCHEDULE/ReportTaxPackage?full=" + (this.isIFrame() ? "true" : "false")+"&moduloAtual=2" /*Modulo 2 representa Tax Package*/, {
					type: "POST",
					xhrFields: {
						withCredentials: true
		...
		_geraRelatorioTax: function (ifExport) {
			...
			const promise1 = function () {
				return new Promise(function (resolve, reject) {
					...
					jQuery.ajax(Constants.urlBackend + "DeepQueryDistinctLOSSSCHEDULE/ReportTaxPackage?full=" + (that.isIFrame() ? "true" : "false")+"&moduloAtual=2" /*Modulo 2 representa Tax Package*/, {
						type: "POST",
						xhrFields: {
							withCredentials: true
						},
			...
			/*
			jQuery.ajax(Constants.urlBackend + "DeepQueryDistinctLOSSSCHEDULE/ReportTaxPackage?full=" + (this.isIFrame() ? "true" : "false")+"&moduloAtual=2" , {
				type: "POST",
				xhrFields: {
					withCredentials: true
				},
