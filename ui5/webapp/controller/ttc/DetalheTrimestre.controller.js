		...
		return BaseController.extend("ui5ns.ui5.controller.ttc.DetalheTrimestre", {
			onInit: function () {
				...
				var oModel = new sap.ui.model.json.JSONModel({
					...
					easyFilterClassificacao: [{
						"classificacao": this.getResourceBundle().getText("viewGeralBorne")
					}, {
						"classificacao": this.getResourceBundle().getText("viewGeralCollected")
					}]
				});
				...
				if (aDocumentosVigentes.length) {
					var dialog = new sap.m.Dialog({
						...
						content: new sap.m.Text({
							textAlign: "Center",
							text: this.getView().getModel("i18n").getResourceBundle().getText("viewTTCErroApagarDocumentos")
						}),
						beginButton: new sap.m.Button({
							text: "Ok",
							press: function () {
								this.getModel().setProperty("/substituirDados", true);
								this._apagarTodosDocumentos();
								this.onImportarDados(eventSource);
								dialog.close();
							}.bind(this)
						}),
			...
			onImportarDados: function (eventSource) {
				var that = this;
			...
			_importarValores: function (bSubstituir, planilha, that) {
				...
				var borne = XLSX.utils.sheet_to_row_object_array(planilha.Sheets["Borne"], {
					'date_format': 'dd/mm/yyyy'
				});
				var collected = XLSX.utils.sheet_to_row_object_array(planilha.Sheets["Collected"], {
					'date_format': 'dd/mm/yyyy'
				});
				...
				for (let i = 0; i < borne.length; i++) {
					...
					if (this.checarLinhaPreenchida(borne[i])) {
						if (borne[i]["Tax KEY"] && borne[i]["Country KEY"] && borne[i]["Fiscal Year KEY"] && borne[i]["Currency KEY"] && valoresCorretos) {
							...
							pagamento.administracao_governamental = borne[i]["Goverment administration/body (payee name)"] ? borne[i][
								"Goverment administration/body (payee name)"
							] : "";
							...
							pagamento["data_pagamento"] = borne[i]["Date of Payment (MM/DD/YYYY)"] ? convertExcelNumberToData(borne[i][
							...
							oErro.title = that.getResourceBundle().getText("viewTTCErroLinha") + " " + (i + 2) + " " + that.getResourceBundle().getText(
								"viewTTCAbaBorne");
				...
				for (let i = 0; i < collected.length; i++) {
					...
					if (this.checarLinhaPreenchida(collected[i])) {
						if (collected[i]["Tax KEY"] && collected[i]["Country KEY"] && collected[i]["Fiscal Year KEY"] && collected[i]["Currency KEY"] &&
							valoresCorretos) {
							pagamento = {};
							let jurisdicao;
							...
							pagamento.administracao_governamental = collected[i]["Goverment administration/body (payee name)"] ? collected[i][
								"Goverment administration/body (payee name)"
							] : "";
							pagamento["fk_jurisdicao.id_dominio_jurisdicao"] = jurisdicao;
							...
							pagamento["data_pagamento"] = collected[i]["Date of Payment (MM/DD/YYYY)"] ? convertExcelNumberToData(collected[i][
								"Date of Payment (MM/DD/YYYY)"
							]) : "";
			...
			checarLinhaPreenchida: function (linha) {
				...
				if (linha["Tax KEY"] || linha["Goverment administration/body (payee name)"] || linha["Jurisdiction"] || linha["Country KEY"] ||
					linha["State"] || linha["City"] || linha["Project"] || linha["Fiscal Year KEY"] || linha["Description"] || linha[
						"Date of Payment (MM/DD/YYYY)"] || linha["Currency"] || linha["Currency KEY"] || linha["Type of Transaction"] || linha[
						"Other Type of Transactions"] || linha["Value"] || linha["Interest"] || linha["Fine"] || linha["Document ID"] || linha[
						"Beneficiary entity"]) {
					retorno = true;
				}
				return retorno;
			...
			onTrocarTax: function (oEvent) {
				...
				if (bPendencia) {
					...
					var dialog = new sap.m.Dialog({
						...
						content: new sap.m.Text({
							textAlign: "Center",
							text: this.getView().getModel("i18n").getResourceBundle().getText("viewTTCErroDocumento")
						}),
						endButton: new sap.m.Button({
							text: "Ok",
							press: function () {
			...
			onFiltrarDocumentos: function () {
				this.filterDialogDocumentos.open();
			},
			...
			_montarFiltro: function (bManterFiltro) {
				...
				Utils.criarDialogFiltro("tableDocumentos", [{
					text: this.getResourceBundle().getText("viewGeralClassificacao"),
					applyTo: 'classificacao',
					items: {
						loadFrom: '',
						path: '/easyFilterClassificacao',
						text: 'classificacao',
						key: 'classificacao'
					}
				}, {
					...
					items: {
						...
						path: '/EasyFilterDocumentosTaxCategory',
						text: 'category',
						key: 'id_tax_category'
					}
				}], this, function (params) {

				}, "filterDialogDocumentos");
				...
				this._loadFrom_filterDialogBorne().then((function (res) {
					...
					res[1].unshift({
						"id_tax_category": 0,
						"category": that.emBrancoTraduzido
					});
					...
					res[3].unshift({
						"id_dominio_jurisdicao": 0,
						"jurisdicao": that.emBrancoTraduzido
					});
					that.getModel().setProperty("/EasyFilterBorneJurisdicao", res[3]);

					res[4].unshift({
						"id_dominio_ano_fiscal": 0,
						"ano_fiscal": that.emBrancoTraduzido
					});
					...
					res[5].unshift({
						"id_dominio_moeda": 0,
						"AcroNome": that.emBrancoTraduzido
					});
					...
					res[6].unshift({
						"id_dominio_tipo_transacao": 0,
						"tipo_transacao": that.emBrancoTraduzido
					});
				...
				this._loadFrom_filterDialogDocumentos().then((function (res) {
					res[0] = Utils.orderByArrayParaBox(res[0], "category");
					res[0].unshift({
						"id_tax_category": 0,
						"category": that.emBrancoTraduzido
					});
					that.getModel().setProperty("/EasyFilterDocumentosTaxCategory", res[0]);
				}));
			},
			...
			onMudarLinhasExibidasDocumentos: function (oEvent, e) {
				var that = this,
					numeroLinhasDocumentos = this.getModel().getProperty("/numeroLinhasDocumentos"),
					isNum = /^\d+$/.test(numeroLinhasDocumentos);
				if (isNum) {
					if (numeroLinhasDocumentos > 50) {
						that._confirmarLinhas();
						return false;
					} else {
						this.byId("tableDocumentos").setVisibleRowCount(Number(numeroLinhasDocumentos));
					}
				}
			},
			...
			_apagarTodosDocumentos: function () {
				var aDocumentosVigentes = this.getModel().getProperty("/documentosVigentes"),
					aDocumentosNaoVigentes = this.getModel().getProperty("/documentosNaoVigentes"),
					novoDocumentoNaoVigente = {};
					
				for (var i = 0; i < aDocumentosVigentes.length; i++) {
					novoDocumentoNaoVigente = {};
